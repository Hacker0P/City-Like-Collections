...
const ProductCard = ({ product, config, onImageClick }) => {
  ...
  return (
    ...
    className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 overflow-hidden flex flex-col h-full hover:shadow-lg transition-all duration-300 group relative cursor-pointer"
>
        {/* Image Section */}
    ...
    />
             ...
             <div className="absolute top-2 right-2 flex flex-col gap-2 z-10">
                 ...
                 >
                     <Heart size={18} className={isWishlisted ? "fill-red-500 text-red-500" : "text-slate-600 dark:text-slate-400"} />
                 </button>
                 <button 
                    onClick={handleShare}
                    className="p-2 bg-white/80 dark:bg-slate-800/80 backdrop-blur-sm rounded-full shadow-sm hover:bg-white dark:hover:bg-slate-700 active:scale-90 transition-all text-slate-700 dark:text-slate-200"
                 >
                     <Share2 size={18} />
                 </button>
             </div>
        ...
        <div className="p-3 flex flex-col flex-1 gap-2">
            <div>
                <h3 className="text-sm font-bold text-slate-900 dark:text-slate-100 leading-tight mb-1 line-clamp-2 min-h-[2rem]">
                    {product.name}
                </h3>
            ...
            <div className="mt-auto pt-2 border-t border-slate-100 dark:border-slate-800 flex gap-2">
                 <button 
    ...
    {showOptionsModal && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-fade-in" onClick={() => setShowOptionsModal(false)}>
            <div className="bg-white dark:bg-slate-900 rounded-2xl w-full max-w-sm p-6 shadow-2xl animate-scale-in" onClick={e => e.stopPropagation()}>
                <div className="flex gap-4 mb-6">
                     ...
                     <div>
                         <h3 className="font-bold text-slate-900 dark:text-slate-100 line-clamp-2 leading-tight">{product.name}</h3>
                ...
                <div className="space-y-6">
                    ...
                    {sizes.length > 0 && (
                        <div>
                            <label className="text-sm font-bold text-slate-700 dark:text-slate-300 mb-2 block">Select Size</label>
                                ...
                                {sizes.map(size => (
                                    <button 
                                        ...
                                        className={`min-w-[40px] h-10 px-3 rounded-lg text-sm font-bold border transition-all ${
                                            selectedSize === size 
                                            ? 'bg-slate-900 text-white border-slate-900 dark:bg-primary-600 dark:border-primary-600 shadow-lg scale-105' 
                    ...
                    {colors.length > 0 && (
                        <div>
                            <label className="text-sm font-bold text-slate-700 dark:text-slate-300 mb-2 block">Select Color</label>
                                ...
                                {colors.map(color => (
                                    <button 
                                        ...
                                        className={`w-10 h-10 rounded-full flex items-center justify-center border transition-all ${
                                            selectedColor === color ? 'border-primary-600 ring-2 ring-primary-100 dark:ring-primary-900 scale-110' : 'border-slate-200 dark:border-slate-700'
                                        }`}
                                    >
                                        <span 
                ...
                <div className="mt-8 flex gap-3">
                    <button 
                        ...
                        className="flex-1 py-3 text-slate-600 dark:text-slate-300 font-bold bg-slate-100 dark:bg-slate-800 rounded-xl hover:bg-slate-200 dark:hover:bg-slate-700 transition-colors"
                    >
                        Cancel
                    </button>
                    <button 
                        ...
                        className="flex-[2] py-3 text-white dark:text-slate-900 font-bold bg-slate-900 dark:bg-white rounded-xl hover:bg-slate-800 dark:hover:bg-slate-100 transition-colors shadow-lg disabled:opacity-50 disabled:cursor-not-allowed"
                    >
                        Confirm & Add
                    </button>
                </div>
